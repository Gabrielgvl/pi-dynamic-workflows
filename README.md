...
| Global | What it does |
| --- | --- |
| `agent(prompt, opts)` | Spawn an isolated subagent; optionally validate its result with a TypeBox schema or the supported literal subset |
...
| `schema` | TypeBox or the literal TypeBox-compatible schema subset documented below |
...
`WorkflowManager.resume(runId, { argsPatch })` performs the same safe shallow patch used by the tool. Set `agentTypePolicy: "error"` to reject unknown named agent definitions instead of using the compatibility fallback.

Structured output accepts TypeBox schemas from the TypeScript API. Literal workflow schemas intentionally support only this TypeBox-compatible schema subset: `type` (`object`, `array`, `string`, `number`, `integer`, `boolean`, or `null`), `properties`, `required`, boolean `additionalProperties`, `items`, primitive `enum`, and `description`. Other standard JSON Schema keywords such as `$ref`, composition keywords, numeric/string bounds, `pattern`, and `format` are not part of the literal public API; use TypeBox when those capabilities are needed.
...
| Structured outputs | TypeBox or the documented literal schema subset, with bounded repair |
