...
The top-level `workflow` tool supports three actions:

- `run` (default): provide exactly one of `script` (raw source) or `scriptPath` (a file rooted at `ctx.cwd`).
- `resume`: provide `runId` and optionally `args` as the same safe shallow JSON-object patch accepted by `WorkflowManager.resume`.
- `status`: provide `runId` to inspect a persisted run without starting or changing it.

Script paths are read fresh for every run and must resolve to a regular file contained by the realpath of `ctx.cwd`; missing files, directories, traversal, and symlink escapes are rejected. The supplied run `args` value is passed unchanged to foreground and background runs.
