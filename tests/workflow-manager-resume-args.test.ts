...
test(
  "resume with an explicit undefined args option preserves persisted input",
  withTempCwd(async (cwd) => {
    const runId = "resume-undefined-args";
    let prompt = "";
    const manager = new WorkflowManager({
      cwd,
      agent: {
    ...
    manager.getPersistence().save(persistedRun(runId, { preserved: true }));
    const completed = waitForComplete(manager, runId);

    assert.equal(await manager.resume(runId, { args: undefined }), true);
    await completed;

    assert.deepEqual(JSON.parse(prompt), { preserved: true });
    assert.deepEqual(manager.getPersistence().load(runId)?.args, { preserved: true });
  }),
);
