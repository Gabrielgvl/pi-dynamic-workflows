...
const caughtAbortScript = `export const meta = { name: 'caught_abort', description: 'caught abort' }
try { await agent('wait', { label: 'wait' }) } catch {}
return 'script-returned-after-abort'`;
...
test(
  "throwing manager agentEnd listeners are best-effort and do not fail or retry the run",
  withTempCwd(async (cwd) => {
    let calls = 0;
    ...
    assert.equal(JSON.stringify(result.result), JSON.stringify({ a: "done" }));
...
test(
  "throwing manager complete listeners are best-effort after journaled success",
  withTempCwd(async (cwd) => {
...
test(
  "caught parallel cancellation leaves no running agents in a completed manager snapshot",
  withTempCwd(async (cwd) => {
    ...
    const script = `export const meta = { name: 'manager-cancel', description: 'manager cancellation' }
try {
  await parallel([() => agent('fatal'), () => agent('sibling')])
} catch {}
return 'completed after catch'`;
    const manager = new WorkflowManager({
      cwd,
      agent: {
    ...
    assert.equal(siblingSettled, true, "parallel waits for sibling cancellation settlement");
    assert.equal(
      run?.agents.some((agent) => agent.status === "running"),
      false,
      "completed manager runs cannot retain running agent snapshots",
    );
    assert.equal(run?.agents.find((agent) => agent.prompt === "sibling")?.status, "error");
  }),
);
...
test(
  "each agent's model records explicit routing without inventing an unapplied mainModel default",
  withTempCwd(async (cwd) => {
...
return { a, b }`;
    ...
    assert.equal(byLabel.judge, undefined, "default agent does not claim an unapplied mainModel route");
  }),
);
...
test(
  "caught script abort cannot overwrite pause lifecycle and the paused generation remains resumable",
  withTempCwd(async (cwd) => {
    let calls = 0;
    ...
    manager.on("error", () => {});
    let completions = 0;
    manager.on("complete", () => completions++);
    const started = new Promise<void>((resolve) => manager.once("agentStart", () => resolve()));
    const { runId, promise } = manager.startInBackground(caughtAbortScript);
    await started;

    assert.equal(manager.pause(runId), true);
    const pausedGeneration = manager.getRun(runId)?.generation;
    releaseFirst();
    ...
    assert.equal(completions, 0, "a caught abort must not emit completion for the paused generation");
    ...
    await new Promise<void>((resolve) => {
      if (manager.getRun(runId)?.status === "completed") resolve();
      else manager.on("complete", (event: { runId: string }) => event.runId === runId && resolve());
    });
    assert.equal(manager.getRun(runId)?.generation, (pausedGeneration ?? 0) + 1);
...
test(
  "caught script abort cannot overwrite stop lifecycle or become resumable",
  withTempCwd(async (cwd) => {
    const da = deferredAgent();
    const manager = new WorkflowManager({ cwd, agent: da.runner });
    manager.on("error", () => {});
...
test(
  "caught external abort cannot complete a synchronous manager run",
  withTempCwd(async (cwd) => {
    const da = deferredAgent();
    ...
    manager.on("complete", () => completions++);
    const started = new Promise<void>((resolve) => manager.once("agentStart", () => resolve()));
    const run = manager.runSync(caughtAbortScript, undefined, { externalSignal: external.signal });
    await started;
...
test(
  "deleteRun never removes a lease owned by another process for a persisted-only run",
  withTempCwd(async (cwd) => {
    const owner = new WorkflowManager({ cwd });
    const runId = "persisted-only-leased-run";
    owner.getPersistence().save({
      runId,
      workflowName: "persisted only",
      script: `export const meta = { name: 'persisted', description: 'persisted' }\nreturn 'done'`,
      status: "completed",
      phases: [],
      agents: [],
      logs: [],
    ...
    const ownerLease = owner.getPersistence().acquireRunLease(runId);
    assert.ok(ownerLease);

    const contender = new WorkflowManager({ cwd });
    assert.equal(contender.getRun(runId), undefined, "the contender does not own the run in memory");
    try {
      assert.equal(contender.deleteRun(runId), false, "lease contention fails cleanly");
      assert.equal(contender.getPersistence().load(runId)?.status, "completed", "persisted state is untouched");
      assert.equal(
        contender.getPersistence().acquireRunLease(runId),
        null,
        "the other process's token-owned lock remains intact",
      );
    } finally {
      owner.getPersistence().releaseRunLease(ownerLease);
    }

    assert.equal(contender.deleteRun(runId), true, "deletion succeeds after acquiring its own lease");
    assert.equal(contender.getPersistence().load(runId), null);
    const released = contender.getPersistence().acquireRunLease(runId);
    assert.ok(released, "the deleting process releases only its own lease token");
    contender.getPersistence().releaseRunLease(released);
  }),
);
