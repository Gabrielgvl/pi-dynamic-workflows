...
test("workflow tool accepts exactly one fresh, cwd-contained scriptPath source", async () => {
  ...
  try {
    ...
    const dottedPath = "workflow..draft.js";
    writeFileSync(join(root, dottedPath), script);
    await execute("call", { scriptPath: dottedPath }, undefined, undefined, { cwd: root });
    assert.equal(captured[2]?.script, script, "valid dotted path components must remain supported");
    ...
    for (const traversalPath of [
      ...
      await assert.rejects(
        () => execute("call", { scriptPath: traversalPath }, undefined, undefined, { cwd: root }),
        /traversal components/,
        `must reject raw traversal before normalization: ${traversalPath}`,
      );
    }
    ...
    const invalid = [
      ...
      [{ scriptPath: relative(root, outsidePath) }, /traversal components/],
    ] as const;
