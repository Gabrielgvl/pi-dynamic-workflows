...
return { prefix, produced: produced.result, consumed }`;
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ["agent"],
      "neither the retained child agent nor its parent workflow wrapper is journaled",
    );
    ...
    const resumed = await runWorkflow(parent, {
      ...
      resumeJournal: new Map(journal.map((entry) => [entry.key ?? entry.index, entry])),
      persistLogs: false,
      agent: runner,
    });
...
const childValue = await workflow(${JSON.stringify(child)}, { handle: args.handle }, { key: 'current-checkout' })
...
return { childValue, suffix }`;
  ...
  try {
    ...
    assert.deepEqual(
      historicalJournal.map((entry) => entry.kind),
      ["workflow", "agent"],
      "the fixture models a wrapper cached by an older runtime plus its suffix",
    );
    ...
    const resumed = await runWorkflow(parent, {
      ...
      resumeJournal: new Map(historicalJournal.map((entry) => [entry.key ?? entry.index, entry])),
...
test("retained handle nested phase identity keeps its durable budget across resume", async () => {
  const { repo, cleanup } = createGitRepo("pi-retained-phase-resume-");
  const child = `export const meta = { name: 'retained_phase_child', description: 'durable retained phase' }
phase('Retained bounded phase', { budget: 100 })
return await agent('child-charge', { label: 'child-charge' })`;
  const parent = `export const meta = { name: 'retained_phase_parent', description: 'resume retained phase accounting' }
const produced = await agent('produce', { label: 'stable-producer', isolation: 'worktree', retainWorktree: true })
await workflow(${JSON.stringify(child)}, { handle: produced.worktree })
return await agent('exhaust-agent-count')`;
  let checkpoint: RuntimeCheckpoint | undefined;
  const chargedRunner = {
    async run(prompt: string, options: AgentRunOptions) {
      if (prompt === "child-charge") {
  ...
  try {
    ...
    const initialPhase = Object.values(checkpoint.phaseBudgets).find(
      (phaseBudget) => phaseBudget.title === "Retained bounded phase",
    );
    assert.equal(initialPhase?.charged, 60);

    await assert.rejects(
      runWorkflow(parent, {
        cwd: repo,
        runId: "durable-retained-phase",
        resumeFromRunId: "durable-retained-phase",
        maxAgents: 2,
        persistLogs: false,
        runtimeCheckpoint: checkpoint,
        agent: chargedRunner,
      }),
      /phase.*Retained bounded phase.*sub-budget exhausted/i,
    );
    assertNoWorktreeLeaks(repo);
  } finally {
    cleanup();
...
return await agent('retained', { isolation: 'worktree', retainWorktree: true })`,
      ...
      assert.equal(persisted?.terminalSnapshot?.outcome, "failed");
...
return 'completed result'`,
      ...
      assert.equal(persisted?.terminalSnapshot?.outcome, "completed");
