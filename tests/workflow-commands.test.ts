...
test("/workflows lifecycle commands reject malformed run IDs with a stable error", async () => {
  for (const command of ["status", "watch", "stop", "pause", "resume", "rm"]) {
    const h = harness();
    await h.run(`${command} ../escape`);
    assert.deepEqual(h.notified, [{ message: "Invalid workflow run ID", type: "error" }], command);
    assert.deepEqual(h.calls, [], `${command} must reject before manager access`);
    assert.equal(h.printed.length, 0, command);
  }
});
