...
test("WorkflowAgent strict resolution fails non-recoverably while non-strict mode falls back", async () => {
  ...
  await assert.rejects(
    () => strictAgent.run("do it", { tier: "unconfigured", modelTierConfig: null }),
    (error) =>
      error instanceof WorkflowError &&
      error.code === WorkflowErrorCode.SCRIPT_VALIDATION_ERROR &&
      /tier.*unconfigured/i.test(error.message),
    "strict mode must reject an explicit tier when neither config nor main-model fallback resolves",
  );
...
test("untagged WorkflowAgent resolves configured medium live and hashes that selected route", async () => {
  const script = `export const meta = { name: 'implicit-medium-live', description: 'live route identity' }
return await agent('constant prompt')`;
  const journal: JournalEntry[] = [];
  const sessions: CreateAgentSessionOptions[] = [];
  const sdkDefault = { provider: "mock", id: "sdk-default", name: "SDK Default" } as Model<any>;
  const mediumOne = { provider: "mock", id: "medium-one", name: "Medium One" } as Model<any>;
  const mediumTwo = { provider: "mock", id: "medium-two", name: "Medium Two" } as Model<any>;
  const settingsManager = {
    getDefaultProvider: () => sdkDefault.provider,
    getDefaultModel: () => sdkDefault.id,
    getDefaultThinkingLevel: () => "low",
  } as any;
  const agent = new WorkflowAgent({
    cwd: "/tmp",
    modelRegistry: registry(sdkDefault, mediumOne, mediumTwo),
  ...
  await runWorkflow(script, {
    agent,
    modelTierConfig: { tiers: { medium: "mock/medium-one" } },
    persistLogs: false,
    onAgentJournal: (entry) => journal.push(entry),
  });
  ...
  await runWorkflow(script, {
    agent,
    modelTierConfig: { tiers: { medium: "mock/medium-two" } },
    persistLogs: false,
    resumeJournal: replayJournal,
  });

  assert.deepEqual(
    sessions.map((options) => options.model && `${options.model.provider}/${options.model.id}`),
    ["mock/medium-one", "mock/medium-two"],
    "live creation and replay hashing must both use the implicit configured medium tier",
  );
});

test("untagged agents use one SDK model and thinking snapshot despite sequential settings drift", async () => {
  const script = `export const meta = { name: 'stable-sdk-defaults', description: 'stable sdk defaults' }
await agent('first')
return await agent('second')`;
  const sdkOne = { provider: "mock", id: "sdk-one", name: "SDK One" } as Model<any>;
  const sdkTwo = { provider: "mock", id: "sdk-two", name: "SDK Two" } as Model<any>;
  let defaultModel = sdkOne;
  let defaultThinking: "low" | "high" = "low";
  let availableModels = [sdkOne, sdkTwo];
  const settingsManager = {
    getDefaultProvider: () => defaultModel.provider,
    getDefaultModel: () => defaultModel.id,
  ...
  const agent = new WorkflowAgent({
    ...
    sessionFactory: async (options) => {
      ...
      if (sessions.length === 1) {
        ...
        availableModels = [sdkTwo];
      }
      const session = {
        model: options.model ?? defaultModel,
        thinkingLevel: options.thinkingLevel ?? defaultThinking,
        systemPrompt: "system prompt",
        messages: [] as unknown[],
        prompt: async () => {
          session.messages.push({ role: "assistant", content: [{ type: "text", text: "done" }] });
  ...
  await runWorkflow(script, { agent, modelTierConfig: null, persistLogs: false });

  assert.deepEqual(
    sessions.map((options) => options.model && `${options.model.provider}/${options.model.id}`),
    ["mock/sdk-one", "mock/sdk-one"],
  );
  assert.deepEqual(
    sessions.map((options) => options.thinkingLevel),
    ["low", "low"],
  );
});
...
test("untagged agents ignore mainModel changes when mainModel is not applied to the live SDK route", async () => {
...
return await agent('constant prompt')`;
  ...
  assert.equal(calls, 1, "an unapplied mainModel display change must not invalidate the actual default route");
});
...
test("untagged replay follows the SDK settings default instead of a stable unapplied mainModel", async () => {
  const script = `export const meta = { name: 'settings-default-route', description: 'settings default route' }
return await agent('constant prompt')`;
  const journal: JournalEntry[] = [];
  const sessions: CreateAgentSessionOptions[] = [];
  const firstModel = { provider: "mock", id: "settings-a", name: "Settings A" } as Model<any>;
  const secondModel = { provider: "mock", id: "settings-b", name: "Settings B" } as Model<any>;
  let defaultModelId = firstModel.id;
  const settingsManager = {
    getDefaultProvider: () => "mock",
    getDefaultModel: () => defaultModelId,
    getDefaultThinkingLevel: () => "low",
  } as any;
  const agent = new WorkflowAgent({
    cwd: "/tmp",
    mainModel: "mock/stable-main-model",
    modelRegistry: registry(firstModel, secondModel),
    session: { settingsManager },
    sessionFactory: fakeSessionFactory(sessions),
    resourceLoaderFactory: () => fakeLoader(),
  });
  ...
  assert.deepEqual(
    sessions.map((options) => options.model && `${options.model.provider}/${options.model.id}`),
    ["mock/settings-a", "mock/settings-b"],
  );
});

test("non-strict unresolved model and tier replay follow the snapshotted fallback model and thinking", async () => {
  const fallbackA = { provider: "mock", id: "fallback-a", name: "Fallback A" } as Model<any>;
  const fallbackB = { provider: "mock", id: "fallback-b", name: "Fallback B" } as Model<any>;
  let defaultModelId = fallbackA.id;
  ...
  const agent = new WorkflowAgent({
    cwd: "/tmp",
    modelRegistry: registry(fallbackA, fallbackB),
    session: { settingsManager },
    sessionFactory: fakeSessionFactory(sessions),
    resourceLoaderFactory: () => fakeLoader(),
  });
  ...
  for (const route of [
    { name: "missing-model", options: "{ model: 'missing-symbol' }", modelTierConfig: null },
    {
      name: "missing-tier-target",
      options: "{ tier: 'small' }",
      modelTierConfig: { tiers: { small: "missing-symbol" } },
    },
  ]) {
    const script = `export const meta = { name: '${route.name}', description: 'fallback replay identity' }
return await agent('constant prompt', ${route.options})`;
    const journal: JournalEntry[] = [];

    defaultModelId = fallbackA.id;
    defaultThinking = "low";
    await runWorkflow(script, {
      agent,
      modelTierConfig: route.modelTierConfig,
      persistLogs: false,
      onAgentJournal: (entry) => journal.push(entry),
    });
    const replayJournal = new Map(journal.map((entry) => [entry.index, entry]));

    defaultModelId = fallbackB.id;
    await runWorkflow(script, {
      agent,
      modelTierConfig: route.modelTierConfig,
      persistLogs: false,
  ...
  assert.deepEqual(
    sessions.map((options) => [
      options.model && `${options.model.provider}/${options.model.id}`,
      options.thinkingLevel,
    ]),
    [
      ["mock/fallback-a", "low"],
      ["mock/fallback-b", "low"],
      ["mock/fallback-a", "high"],
      ["mock/fallback-a", "low"],
      ["mock/fallback-b", "low"],
      ["mock/fallback-a", "high"],
    ],
...
test("settings default thinking participates in replay identity when no thinking is explicit", async () => {
  const script = `export const meta = { name: 'settings-thinking-route', description: 'settings thinking identity' }
return await agent('constant prompt')`;
  const journal: JournalEntry[] = [];
  const observedThinking: string[] = [];
  let defaultThinking = "low";
  const settingsManager = {
    getDefaultProvider: () => fallbackModel.provider,
    getDefaultModel: () => fallbackModel.id,
    getDefaultThinkingLevel: () => defaultThinking,
  } as any;
  const sessionFactory = async (options: CreateAgentSessionOptions) => {
    const thinkingLevel = options.thinkingLevel ?? options.settingsManager?.getDefaultThinkingLevel?.() ?? "medium";
    observedThinking.push(thinkingLevel);
    const session = {
  ...
  const agent = new WorkflowAgent({
    cwd: "/tmp",
    modelRegistry: registry(fallbackModel),
    session: { settingsManager },
    sessionFactory,
    resourceLoaderFactory: () => fakeLoader(),
  });
  ...
  assert.deepEqual(observedThinking, ["low", "high"], "changing the live SDK/settings default must rerun the agent");
});

test("schema-repair telemetry preserves the work-phase active coding tools", async () => {
  const telemetry: AgentTelemetry[] = [];
  ...
  const sessionFactory = async (options: CreateAgentSessionOptions) => {
    const session = {
      ...
      messages: [] as unknown[],
      prompt: async () => {
        session.messages.push({ role: "assistant", content: [{ type: "text", text: '{"word":"ok"}' }] });
      },
  ...
  assert.deepEqual(
    await agent.run("return a word", {
      schema: Type.Object({ word: Type.String() }),
      maxSchemaRetries: 1,
      modelTierConfig: null,
      onTelemetry: (value) => telemetry.push(value),
    }),
  ...
  assert.deepEqual(activeTools, ["structured_output"], "the repair turn should actually restrict active tools");
  assert.deepEqual(telemetry[0].activeToolNames, ["read", "bash"]);
  assert.equal(telemetry[0].activeToolCount, 2);
});
