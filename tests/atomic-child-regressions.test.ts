...
test("atomic child phase budgets stay isolated and resume under a deterministic invocation namespace", async () => {
  const childScript = `export const meta = { name: 'child', description: 'child' }
phase('Shared', { budget: 100 })
return await agent('child-agent')`;
  const script = parentScript(`phase('Shared', { budget: 100 })
await agent('parent-before')
await workflow('child')
await agent('parent-after')
return 'done'`);
  const initialPhaseBudgets = {
    Shared: { budget: 100, spent: 40, warned: false },
  };
  const runOnce = async (restored: Record<string, { budget: number; spent: number; warned: boolean }>) => {
    const progress: Record<string, { budget: number; spent: number; warned: boolean }> = {};
    await runWorkflow(script, {
      runId: "phase-isolation",
      persistLogs: false,
      loadSavedWorkflow: () => childScript,
      initialPhaseBudgets: restored,
      onPhaseBudgetProgress: (key, budget) => {
        progress[key] = budget;
      },
      agent: {
        async run(_prompt: string, options: any) {
          options.onUsage?.({ input: 10, output: 0, cacheRead: 0, cacheWrite: 0, total: 10, cost: 0 });
          return "ok";
        },
      },
    });
  ...
  const first = await runOnce(initialPhaseBudgets);
  assert.deepEqual(first.Shared, { budget: 100, spent: 60, warned: false });
  const childKeys = Object.keys(first).filter((key) => key !== "Shared");
  assert.equal(childKeys.length, 1, "child progress must use one invocation-scoped persistence key");
  const childKey = childKeys[0];
  assert.match(childKey, /phase-isolation-child1/);
  assert.deepEqual(first[childKey], { budget: 100, spent: 10, warned: false });

  const second = await runOnce(first);
  assert.deepEqual(second.Shared, { budget: 100, spent: 80, warned: false });
  assert.deepEqual(second[childKey], { budget: 100, spent: 20, warned: false });
  assert.equal(
    Object.keys(second).find((key) => key !== "Shared"),
    childKey,
    "the child phase namespace must be deterministic across cold resume",
  );
});
...
test("atomic child attempts persist cumulative usage before the parent journal commits", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pi-dw-child-usage-progress-"));
  const home = mkdtempSync(join(tmpdir(), "pi-dw-child-usage-progress-home-"));
  try {
    await withFakeHomeAsync(home, async () => {
      const childScript = `export const meta = { name: 'child', description: 'child' }
await agent('first-child')
return await agent('second-child')`;
      const script = parentScript("return await workflow('child')");
        ...
        agent: {
          async run(prompt: string, options: any) {
            if (prompt === "first-child") {
              options.onUsage?.({ input: 7, output: 0, cacheRead: 0, cacheWrite: 0, total: 7, cost: 0 });
              return "first";
            }
            const duringAtomicChild = manager.getPersistence().load(runId);
            assert.equal(duringAtomicChild?.status, "running");
            assert.equal(duringAtomicChild?.journal.length, 0, "the atomic parent entry must still be uncommitted");
            assert.equal(
              duringAtomicChild?.tokenUsage?.total,
              7,
              "the finalized child attempt must already be durable for crash recovery",
            );
            options.onUsage?.({ input: 3, output: 0, cacheRead: 0, cacheWrite: 0, total: 3, cost: 0 });
      ...
      assert.equal(paused?.journal.length, 0);
      assert.equal(paused?.tokenUsage?.total, 10, "each finalized attempt is counted and persisted exactly once");
    });
  } finally {
    rmSync(cwd, { recursive: true, force: true });
    rmSync(home, { recursive: true, force: true });
