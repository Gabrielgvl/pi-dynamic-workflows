...
import { MAX_AGENTS_PER_RUN, normalizeMaxAgents } from "./config.js";
import {
  ...
  return defineTool({
    ...
    get promptGuidelines() {
      return [
        ...
        "For workflow structured output, opts.schema supports only the literal TypeBox-compatible subset: type, properties, required, additionalProperties, items, enum, description. TypeBox constructors are API-only.",
...
function normalizeWorkflowToolArgs(args: unknown): WorkflowToolInput {
  ...
  if (action === "run") {
    ...
    return {
      ...
      ...(Object.hasOwn(value, "args") ? { args: normalizeWorkflowArgs(value.args) } : {}),
      ...(Object.hasOwn(value, "maxAgents") ? { maxAgents: normalizeMaxAgents(value.maxAgents) } : {}),
...
function normalizeWorkflowArgs(args: unknown): unknown {
  ...
  try {
    ...
    throw new Error("workflow `args` must contain only deterministic JSON values");
  }
}
