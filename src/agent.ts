...
export class WorkflowAgent {
  ...
  /**
   * Snapshot the same default model that createAgentSession will select when a
   * call supplies no explicit model. The workflow runtime uses this both as the
   * live route and as replay identity, so an unrelated mainModel display value
   * cannot mask a settings-default change.
   */
  async resolveSessionDefaultModelSpec(perRunRegistry?: ModelRegistry): Promise<string | undefined> {
    if (this.sessionOptions.model) return canonicalModelSpec(this.sessionOptions.model);
    ...
    if (provider && modelId) {
      ...
      if (configured && modelRegistry.hasConfiguredAuth(configured)) return canonicalModelSpec(configured);
    }

    const available = await modelRegistry.getAvailable();
    const fallback = selectSdkDefaultAvailableModel(available);
    return fallback ? canonicalModelSpec(fallback) : undefined;
  }
