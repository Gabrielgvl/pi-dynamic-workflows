...
export function formatWorkflowReport(run: PersistedRunState): string {
  ...
  if (aggregate && !usageUnavailable) {
    ...
    const legacy = run.tokenUsage;
    const cache =
      legacy.cacheRead === undefined || legacy.cacheWrite === undefined
        ? "cache unavailable"
        : `cache read ${number(legacy.cacheRead)} · cache write ${number(legacy.cacheWrite)} · cache ${cacheRatio({ input: legacy.input, cacheRead: legacy.cacheRead })}`;
    lines.push(
      `Usage: input ${number(legacy.input)} · output ${number(legacy.output)} · ${cache} · total ${number(legacy.total)} · ${legacy.cost === undefined ? "cost unavailable" : cost(legacy.cost)}`,
    );
  } else {
  ...
  for (const agent of run.agents) {
    ...
    if (!hasCapturedTelemetry) {
      lines.push(
        `[${agent.id}] ${agent.label} · ${telemetry.execution} · requested ${telemetry.requestedModelSpec ?? "(default)"} · telemetry unavailable`,
      );
      continue;
    }
