...
export async function runWorkflow<T = unknown>(
  ...
  const sessionDefaultModel =
    ...
    (agentRunner instanceof WorkflowAgent
      ? await agentRunner.resolveSessionDefaultModelSpec(options.modelRegistry)
      : undefined);
  const applySessionDefaultModel = agentRunner instanceof WorkflowAgent && !agentRunner.hasExplicitSessionModel();
  ...
  const agent = async (prompt: string, agentOptions: AgentOptions = {}) => {
    ...
    // An untagged/unrouted call is pinned to the SDK default selected from the
    // session/settings snapshot. mainModel remains only the explicit-tier fallback.
    const modelSpec =
      explicitModel ?? phaseModel ?? (agentOptions.tier || !applySessionDefaultModel ? undefined : sessionDefaultModel);
    // For display in /workflows: the model this agent actually routes to. The real
    // resolved id overrides this via onModelResolved once the session is created.
    let displayModel = modelSpec ?? (agentOptions.tier ? effectiveMainModel : sessionDefaultModel);
