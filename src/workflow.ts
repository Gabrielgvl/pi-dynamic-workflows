...
import { canonicalizeWorkflowCwd } from "./cwd.js";
...
export interface JournalEntry {
  index: number;
  /** Phase charged for this live agent. Added for phase-budget resume reconstruction. */
  phase?: string;
...
export interface SharedRuntime {
  ...
  /** @deprecated Nesting depth is invocation-local; retained for public API compatibility. */
  depth: number;
}

export interface WorkflowPhaseBudget {
  budget: number;
  spent: number;
  warned: boolean;
}
...
export interface WorkflowRunOptions extends WorkflowAgentOptions {
  ...
  /** Internal: invocation-local workflow() nesting depth. */
  ...
  /** Internal: cumulative per-phase budget state restored before a cold resume. */
  initialPhaseBudgets?: Readonly<Record<string, WorkflowPhaseBudget>>;
  /** Internal: called whenever a phase budget is declared, warned, or charged. */
  onPhaseBudgetProgress?: (title: string, budget: WorkflowPhaseBudget) => void;
  /** Internal: tier configuration snapshotted by the parent atomic workflow. */
  modelTierConfig?: ModelTierConfig | null;
  /** Internal: SDK/settings defaults snapshotted once by the top-level workflow. */
  sessionDefaultsSnapshot?: WorkflowSessionDefaults;
  /** Internal: model availability snapshotted once by the top-level workflow. */
  ...
  /** Public phase observer. Invoked only with declared/restored phase titles. */
  onPhase?: (title: string) => void;
  /** Internal lifecycle state observer; unlike onPhase, an absent restored phase is meaningful. */
  onPhaseState?: (title: string | undefined) => void;
  onAgentStart?: (event: { callId?: string; label: string; phase?: string; prompt: string; model?: string }) => void;
  /** Best-effort terminal diagnostic; listener errors never change workflow outcome or transaction state. */
  onAgentEnd?: (event: {
    callId?: string;
    label: string;
    phase?: string;
  ...
  /** Best-effort final diagnostic; listener errors never change a completed workflow outcome. */
...
export interface AgentOptions<TSchemaDef extends WorkflowSchema | undefined = WorkflowSchema | undefined> {
  ...
  phase?: string;
  /**
   * TypeBox schema, or the explicit literal subset described by `LiteralJsonSchema`.
   * Arbitrary/full JSON Schema objects are not part of the public contract.
   */
  schema?: TSchemaDef;
  /**
...
interface RuntimeState {
  currentPhase?: string;
  /** Per-phase soft sub-budgets, including cumulative spend restored on resume. */
  phaseBudgets: Map<string, WorkflowPhaseBudget>;
  logs: string[];
  phases: string[];
...
export async function runWorkflow<T = unknown>(
  ...
  // mainModel is only the fallback for an explicitly requested but unconfigured
  // tier. Untagged agents use the SDK/session default snapshotted below.
  const effectiveMainModel = options.mainModel ?? sessionModel;
  const maxAgents = options.maxAgents === undefined ? MAX_AGENTS_PER_RUN : normalizeMaxAgents(options.maxAgents);
  const agentTimeoutMs = options.agentTimeoutMs !== undefined ? options.agentTimeoutMs : DEFAULT_AGENT_TIMEOUT_MS;
  const runId = options.runId ?? `run-${started.toString(36)}`;
  assertValidRunId(runId);
  const baseCwd = canonicalizeWorkflowCwd(options.cwd ?? process.cwd());
  const agentTypePolicy = options.agentTypePolicy ?? "fallback";
  let workflowArgs: unknown;
  if (options.args !== undefined) {
    try {
      workflowArgs = normalizeJsonTree(options.args);
    } catch {
      throw scriptValidationError("workflow args must contain only deterministic JSON values");
    }
  }
  ...
  const state: RuntimeState = {
    ...
    phaseBudgets: new Map(
      Object.entries(options.initialPhaseBudgets ?? {}).flatMap(([title, value]) => {
        // Child progress is persisted beside parent progress but never admitted
        // into the parent's live phase map. The deterministic child invocation
        // extracts only its own namespace when it starts.
        if (title.startsWith(CHILD_PHASE_BUDGET_PREFIX)) return [];
        const normalized = normalizePhaseBudget(value);
        return normalized ? [[title, normalized] as const] : [];
      }),
    ),
    callSeq: 0,
  ...
  const agentRunner =
    options.agent ?? new WorkflowAgent({ ...options, cwd: baseCwd, modelAliases, strictModelResolution });
  const runnerModelRegistry =
    options.modelRegistry ?? (agentRunner instanceof WorkflowAgent ? agentRunner.getModelRegistry() : undefined);
  const sessionDefaults =
    options.sessionDefaultsSnapshot ??
    (agentRunner instanceof WorkflowAgent
      ? await agentRunner.resolveSessionDefaults(runnerModelRegistry)
      : {
          modelSpec: sessionModel,
          model: options.session?.model,
          thinkingLevel: options.session?.thinkingLevel ?? "medium",
        });
  const sessionDefaultModel = sessionModel ?? sessionDefaults.modelSpec;
  const sessionDefaultThinking = options.session?.thinkingLevel ?? sessionDefaults.thinkingLevel;
  const modelSnapshot = options.modelSnapshot
    ? [...options.modelSnapshot]
    : runnerModelRegistry
      ? [...runnerModelRegistry.getAll()]
      : [];
  if (
    runnerModelRegistry &&
    sessionDefaults.model &&
    !modelSnapshot.some(
      (model) => model.provider === sessionDefaults.model?.provider && model.id === sessionDefaults.model?.id,
    )
  ) {
    modelSnapshot.push(sessionDefaults.model);
  }
  const routingModelRegistry = modelSnapshot.length > 0 ? { getAll: () => [...modelSnapshot] } : undefined;
  const concurrency = normalizeConcurrency(
    options.concurrency ?? Math.max(1, (globalThis.navigator?.hardwareConcurrency ?? 8) - 2),
  );
  const runReplayIdentity = JSON.stringify({
    args: normalizedReplayValue(workflowArgs),
    cwd: baseCwd,
    maxAgents,
    agentTimeoutMs,
  ...
  const nestedWorkflowDepth = options.nestedWorkflowDepth ?? 0;
  ...
  const notifyAgentEnd = (event: Parameters<NonNullable<WorkflowRunOptions["onAgentEnd"]>>[0]) => {
    try {
      options.onAgentEnd?.(event);
    } catch {
      // Terminal observers are diagnostics only. In particular, a listener must
      // not turn an already-journaled and committed success into a retry/failure.
    }
  };
  ...
  const phase = (title: string, phaseOptions?: { budget?: number }) => {
    ...
    // Re-declaration updates the configured ceiling but never refreshes spend
    // restored from an earlier generation.
    if (typeof phaseOptions?.budget === "number" && phaseOptions.budget > 0) {
      const restored = state.phaseBudgets.get(title);
      const phaseBudget = {
        budget: phaseOptions.budget,
        spent: restored?.spent ?? 0,
        warned: restored?.warned ?? false,
      };
      state.phaseBudgets.set(title, phaseBudget);
      options.onPhaseBudgetProgress?.(title, { ...phaseBudget });
    }
    options.onPhaseState?.(title);
  ...
  const agent = async (prompt: string, agentOptions: AgentOptions = {}) => {
    ...
    // Leave untagged calls unpinned so WorkflowAgent.resolveAgentModelSpec can
    // apply the configured implicit medium tier. The settings/SDK default is a
    // replay/display fallback only; pre-pinning it here would bypass tier routing.
    const modelSpec = explicitModel ?? phaseModel;
    // For display in /workflows: snapshot the same effective route used in the
    // replay hash. The live resolved id still overrides this after creation.
    let displayModel =
      resolveAgentModelSpec({ model: modelSpec, tier: agentOptions.tier }, effectiveMainModel, () => modelTierConfig) ??
      sessionDefaultModel;
    ...
    const routeIdentity = effectiveRouteIdentity({
      ...
      inheritedThinking: sessionDefaultThinking,
      modelRegistry: routingModelRegistry,
    });
    ...
    // other's journaled delta. Composing the invocation's runId with callIndex
    // makes the key unique across the whole store.
    ...
    if (hashMatches && !cachedEmptyOutput && callIndex < state.firstMiss) {
      let replayResult = cached.result;
      let diagnosticResult = cached.result;
      if (agentOptions.schema !== undefined) {
        try {
          replayResult = normalizeJsonTree(cached.result);
          diagnosticResult = normalizeJsonTree(cached.result);
        } catch {
          throw scriptValidationError("replayed structured agent result must contain only deterministic JSON values");
        }
      }
      state.callSeq++;
      shared.agentCount++;
    ...
    // Per-phase soft sub-budget gates only live work. Replayed calls were already
    // charged in the restored phase spend and must remain traversable so scripts
    // can reach their caught phase-exhaustion branch on every resume.
    if (assignedPhase) {
      const pb = state.phaseBudgets.get(assignedPhase);
      if (pb) {
        const phaseSpent = pb.spent;
        ...
        if (!pb.warned && phaseSpent >= pb.budget * 0.8) {
          pb.warned = true;
          options.onPhaseBudgetProgress?.(assignedPhase, { ...pb });
    ...
    state.callSeq++;
    // Reserve the live agent slot synchronously after all gates. No await occurs
    // before this increment, so parallel fan-out still cannot overshoot maxAgents.
    shared.agentCount++;
    localAgentCount++;
    ...
    return limiter(async () => {
      ...
      const endAgent = (event: Parameters<NonNullable<WorkflowRunOptions["onAgentEnd"]>>[0]) => {
        if (agentEnded) return;
        agentEnded = true;
        notifyAgentEnd(event);
      };
      ...
      const recordTokens = (result: unknown, usage: AgentUsage | undefined): number => {
        ...
        shared.spent += tokens;
        if (assignedPhase) {
          const phaseBudget = state.phaseBudgets.get(assignedPhase);
          if (phaseBudget) {
            phaseBudget.spent += tokens;
            options.onPhaseBudgetProgress?.(assignedPhase, { ...phaseBudget });
          }
        }
        return tokens;
      };
      ...
      try {
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          ...
          let providerOrSessionStarted = agentRunner.constructor !== WorkflowAgent;
          let accountedTokens = 0;
          let journalFailure: unknown;
          let usageProgressFailure: unknown;
          let runnerSettled = true;
          const persistUsageProgress = () => {
            try {
              // Publish only after this attempt's accounting is finalized. The
              // manager persists synchronously, so parallel completions serialize
              // into monotonic cumulative snapshots without write races.
              options.onTokenUsageProgress?.({ ...shared.tokenUsage });
            } catch (error) {
              usageProgressFailure = error;
              throw error;
            }
          };
          ...
          try {
            ...
            const runnerPromise = agentRunner.run(prompt, {
              ...
              tier: agentOptions.tier,
              effort: agentOptions.effort,
              modelRegistry: runnerModelRegistry,
              modelTierConfig,
              sessionDefaultModelSpec: sessionDefaultModel,
              sessionDefaultModel: sessionDefaults.model,
              sessionDefaultThinkingLevel: sessionDefaultThinking,
              modelSnapshot,
              modelAliases,
              ...
              cwd: runCwd,
              onSessionStart: () => {
                if (callbacksOpen) providerOrSessionStarted = true;
              },
            ...
            try {
              ...
              runnerSettled = await waitForRunnerSettlement(runnerPromise, RUNNER_CLEANUP_GRACE_MS);
              finalizeAttempt(runnerSettled);
              throw error;
            }
            ...
            let journalResult = result;
            let returnedResult = result;
            let diagnosticResult = result;
            if (agentOptions.schema !== undefined) {
              try {
                journalResult = normalizeJsonTree(result, scriptJsonPrototypes);
                returnedResult = normalizeJsonTree(journalResult);
                diagnosticResult = normalizeJsonTree(journalResult);
              } catch {
                throw scriptValidationError("structured agent result must contain only deterministic JSON values");
              }
            }

            accountedTokens = recordTokens(result, accountedUsage);
            usageRecorded = true;
            persistUsageProgress();
            ...
            if (!usageRecorded && (accountedUsage || !executionSignal.aborted)) {
              accountedTokens = providerOrSessionStarted || accountedUsage ? recordTokens(null, accountedUsage) : 0;
              usageRecorded = true;
              persistUsageProgress();
            }
            if (usageProgressFailure !== undefined) throw usageProgressFailure;
            ...
            const retryWouldOverlapSetup = !runnerSettled && agentRunner.constructor === WorkflowAgent;
            if (workflowError.recoverable && attempt < maxAttempts && !retryWouldOverlapSetup) {
              log(
            ...
            if (workflowError.recoverable && attempt < maxAttempts && retryWouldOverlapSetup) {
              log(
                `agent "${label}" attempt ${attempt}/${maxAttempts} did not settle after cancellation; skipping retries to prevent overlapping setup`,
              );
            }
        ...
        if (!agentEnded) {
          const workflowError = executionSignal.aborted
  ...
  const workflowFn = async (nameOrDescriptor: string | WorkflowScriptDescriptor, ...childArgList: unknown[]) => {
    const invocationSignal = currentSignal();
    const parentPhase = state.currentPhase;
    throwIfAborted(invocationSignal);
    if (nestedWorkflowDepth >= 1) {
    ...
    if (hashMatches && callIndex < state.firstMiss) {
      ...
      if (cached.storeDelta) store.applyDelta(cached.storeDelta, cached.storeVersions);
      options.onPhaseState?.(parentPhase);
      if (parentPhase !== undefined) options.onPhase?.(parentPhase);
      return replayResult;
    }
    ...
    const childRunId = createChildRunId(runId, callIndex);
    let childFailure: unknown;
    const restoreParentPhase = () => {
      options.onPhaseState?.(parentPhase);
      if (parentPhase !== undefined) options.onPhase?.(parentPhase);
    };
    try {
      // Never spread invocation-local state or callbacks into an atomic child.
      // In particular, a parent and child may use the same human phase title;
      // their budgets must remain separate while child progress is still durable
      // across a crash before the atomic parent journal entry commits.
      const {
        args: _parentArgs,
        runId: _parentRunId,
        signal: _parentSignal,
        persistLogs: _parentPersistLogs,
        resumeJournal: _parentResumeJournal,
        resumeFromRunId: _parentResumeFromRunId,
        requireJsonResult: _parentRequireJsonResult,
        sharedRuntime: _parentSharedRuntime,
        nestedWorkflowDepth: _parentNestedWorkflowDepth,
        initialTokenUsage: _parentInitialTokenUsage,
        initialPhaseBudgets: _parentInitialPhaseBudgets,
        sharedStore: _parentSharedStore,
        modelTierConfig: _parentModelTierConfig,
        sessionDefaultsSnapshot: _parentSessionDefaultsSnapshot,
        modelSnapshot: _parentModelSnapshot,
        mainModel: _parentMainModel,
        modelAliases: _parentModelAliases,
        strictModelResolution: _parentStrictModelResolution,
        agentRegistry: _parentAgentRegistry,
        loadSavedWorkflow: _parentLoadSavedWorkflow,
        confirm: _parentConfirm,
        onAgentJournal: _parentOnAgentJournal,
        onLog: _parentOnLog,
        onPhase: _parentOnPhase,
        onPhaseState: _parentOnPhaseState,
        onAgentStart: _parentOnAgentStart,
        onAgentEnd: _parentOnAgentEnd,
        onAgentHistory: _parentOnAgentHistory,
        onTokenUsageProgress: _parentOnTokenUsageProgress,
        onTokenUsage: _parentOnTokenUsage,
        onPhaseBudgetProgress: _parentOnPhaseBudgetProgress,
        ...inheritedChildOptions
      } = options;
      const child = await runWorkflow(resolved.script, {
        ...
        agentRegistry,
        modelTierConfig,
        sessionDefaultsSnapshot: sessionDefaults,
        modelSnapshot,
        sharedRuntime: shared,
        nestedWorkflowDepth: nestedWorkflowDepth + 1,
        signal: invocationSignal,
        initialPhaseBudgets: childPhaseBudgets(options.initialPhaseBudgets, childRunId),
        onPhaseBudgetProgress: (title, phaseBudget) => {
          options.onPhaseBudgetProgress?.(childPhaseBudgetKey(childRunId, title), phaseBudget);
        },
        // Child usage is part of the shared run budget and must be persisted even
        // while the parent-level atomic journal entry is still absent.
        onTokenUsageProgress: options.onTokenUsageProgress,
        onLog: options.onLog,
        onPhase: options.onPhase,
        onPhaseState: options.onPhaseState,
        onAgentHistory: options.onAgentHistory,
      ...
      try {
        if (childFailure === undefined) {
          ...
          try {
            ...
            // The original child failure remains authoritative.
          }
        }
      } finally {
        childStore.dispose();
  ...
  const checkpoint = async (promptText: string, checkpointOptions: CheckpointOptions = {}) => {
    ...
    const callIndex = state.callSeq++;
    const hasDefault = Object.hasOwn(checkpointOptions, "default");
    let defaultSnapshot: unknown;
    let normalizedDefault: unknown = { kind: "absent" };
    if (hasDefault) {
      try {
        defaultSnapshot = normalizeJsonTree(checkpointOptions.default, scriptJsonPrototypes);
        normalizedDefault = { kind: "json", value: defaultSnapshot };
      } catch {
        throw scriptValidationError("checkpoint default must contain only deterministic JSON values");
      }
    }
    let callHash: string;
    try {
      callHash = hashCheckpoint(
        promptText,
        checkpointOptions,
        normalizedDefault,
        runReplayIdentity,
        scriptJsonPrototypes,
      );
    } catch {
      throw scriptValidationError("checkpoint options must contain only deterministic JSON values");
    }
    const cached = options.resumeJournal?.get(callIndex);
    if (cached != null && cached.hash === callHash && callIndex < state.firstMiss) {
      let replayedReply: unknown;
      try {
        replayedReply = normalizeJsonTree(cached.result);
      } catch {
        throw scriptValidationError("replayed checkpoint reply must contain only deterministic JSON values");
      }
      shared.agentCount++;
      localAgentCount++;
    ...
    if (options.confirm) {
      ...
      reply = hasDefault ? defaultSnapshot : true;
    }
    throwIfAborted();
    let journaledReply: unknown;
    let returnedReply: unknown;
    try {
      journaledReply = normalizeJsonTree(reply, scriptJsonPrototypes);
      returnedReply = normalizeJsonTree(journaledReply);
    } catch {
      throw scriptValidationError("checkpoint reply must contain only deterministic JSON values");
    }
    options.onAgentJournal?.({ index: callIndex, hash: callHash, result: journaledReply });
  ...
  try {
    ...
    // Emit final token usage as a best-effort diagnostic. Attempt-level usage
    // persistence remains transactional through onTokenUsageProgress above.
    try {
      options.onTokenUsage?.(shared.tokenUsage);
    } catch {
      // Observer failures must not invalidate an already-journaled workflow.
    }
...
export function resolveWorkflowScriptPath(
  ...
  // Reject traversal in the raw input, before node:path normalizes it away.
  // Treat both POSIX and Windows separators as separators on every platform so
  // a descriptor cannot become unsafe when moved between hosts.
  if (/(?:^|[\\/])\.\.(?=$|[\\/])/.test(scriptPath)) {
    throw scriptValidationError("workflow() scriptPath must not contain '..' traversal components");
  }
...
function createChildRunId(parentRunId: string, callIndex: number): string {
  const untruncated = `${parentRunId}-child${callIndex}`;
  if (untruncated.length <= 128) return untruncated;

  const digest = createHash("sha256").update(`${parentRunId}:${callIndex}`).digest("hex").slice(0, 16);
  const suffix = `-child${callIndex}-${digest}`;
  return `${parentRunId.slice(0, 128 - suffix.length)}${suffix}`;
}

const CHILD_PHASE_BUDGET_PREFIX = "@workflow-child-phase/";

function childPhaseBudgetKey(childRunId: string, title: string): string {
  return `${CHILD_PHASE_BUDGET_PREFIX}${encodeURIComponent(childRunId)}/${encodeURIComponent(title)}`;
}

function childPhaseBudgets(
  persisted: Readonly<Record<string, WorkflowPhaseBudget>> | undefined,
  childRunId: string,
): Record<string, WorkflowPhaseBudget> {
  const prefix = `${CHILD_PHASE_BUDGET_PREFIX}${encodeURIComponent(childRunId)}/`;
  const restored = Object.create(null) as Record<string, WorkflowPhaseBudget>;
  for (const [key, budget] of Object.entries(persisted ?? {})) {
    if (!key.startsWith(prefix)) continue;
    try {
      const title = decodeURIComponent(key.slice(prefix.length));
      const normalized = normalizePhaseBudget(budget);
      if (normalized) restored[title] = normalized;
    } catch {
      // Ignore malformed reserved keys instead of letting unrelated persisted
      // state prevent the deterministic child invocation from running.
    }
  }
  return restored;
}

function normalizePhaseBudget(value: unknown): WorkflowPhaseBudget | undefined {
  if (!value || typeof value !== "object") return undefined;
  const budget = (value as Partial<WorkflowPhaseBudget>).budget;
  const spent = (value as Partial<WorkflowPhaseBudget>).spent;
  const warned = (value as Partial<WorkflowPhaseBudget>).warned;
  if (!Number.isFinite(budget) || (budget as number) <= 0) return undefined;
  if (!Number.isFinite(spent) || (spent as number) < 0) return undefined;
  return { budget: budget as number, spent: spent as number, warned: warned === true };
}
...
function normalizedReplayValue(value: unknown, prototypes?: JsonTreePrototypes): unknown {
  if (value === undefined) return { kind: "undefined" };
  return { kind: "json", value: normalizeJsonTree(value, prototypes) };
}
...
function effectiveRouteIdentity(options: {
  ...
  modelRegistry?: Pick<NonNullable<WorkflowAgentOptions["modelRegistry"]>, "getAll">;
}): string {
  const requested =
    resolveAgentModelSpec(
  ...
  const fallback =
    !options.strict && requested && options.modelRegistry && !resolved?.model
      ? { model: options.sessionModel ?? null, thinking: thinking ?? null }
      : null;
  return JSON.stringify({
...
function modelRegistrySnapshotKey(
  registry: Pick<NonNullable<WorkflowAgentOptions["modelRegistry"]>, "getAll"> | undefined,
): unknown {
...
function hashCheckpoint(
  ...
  const identity = JSON.stringify({
    ...
    headless: normalizedReplayValue(options.headless ?? "default", prototypes),
    kind: normalizedReplayValue(options.kind ?? "confirm", prototypes),
    choices: normalizedReplayValue(options.choices, prototypes),
    timeoutMs: normalizedReplayValue(options.timeoutMs, prototypes),
    runIdentity,
  });
