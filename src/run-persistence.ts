...
export interface PersistedPhaseBudget {
  budget: number;
  spent: number;
  warned: boolean;
}
...
export interface PersistedRunState {
  ...
  phases: string[];
  currentPhase?: string;
  /** Cumulative phase-budget accounting. Optional only for legacy records. */
      ...
      for (const dir of [runsDir, legacyRunsDir]) {
        try {
          ...
          for (const file of files) {
            try {
              const parsed = JSON.parse(_readFileSync(join(dir, file), "utf-8")) as unknown;
              if (
                typeof parsed !== "object" ||
                parsed === null ||
                typeof (parsed as { runId?: unknown }).runId !== "string" ||
                !RUN_ID_PATTERN.test((parsed as { runId: string }).runId)
              ) {
                console.warn("[workflow] skipping persisted run with an invalid run ID");
                continue;
              }
