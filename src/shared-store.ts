...
export class SharedStore {
  ...
  /** Retrieve the highest-version observable value across this scope and its parent. */
  get(key: string): unknown {
    this.assertOpen();
    validateStoreKey(key);
    return this.latestWrite(key)?.value;
  }

  /** Whether `key` is present locally or in the parent overlay. */
  has(key: string): boolean {
    this.assertOpen();
    validateStoreKey(key);
    return this.latestWrite(key) !== undefined;
  }
  ...
  private latestWrite(key: string): StoreWrite | undefined {
    this.assertOpen();
  ...
  private allWrites(): StoreWrite[] {
    this.assertOpen();
    return [...(this.parent?.allWrites() ?? []), ...this.writes];
  }
