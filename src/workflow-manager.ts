...
import { MAX_AGENT_RETRIES, MAX_AGENTS_PER_RUN, MAX_CONCURRENCY, normalizeMaxAgents } from "./config.js";
...
export interface ManagedRun {
  ...
  /** Cumulative per-phase budget accounting, durable across generations. */
  phaseBudgets: Record<string, PersistedPhaseBudget>;
...
function normalizeWorkflowArgs(args: unknown): unknown {
  ...
  try {
    return normalizeJsonTree(args);
  } catch {
    throw resumeValidationError("workflow args must contain only deterministic JSON values");
  }
}

function createPhaseBudgetRecord(): Record<string, PersistedPhaseBudget> {
  return Object.create(null) as Record<string, PersistedPhaseBudget>;
}
...
function restorePhaseBudgets(persisted: PersistedRunState): Record<string, PersistedPhaseBudget> {
  const restored = createPhaseBudgetRecord();
  if (persisted.phaseBudgets) {
    for (const [title, value] of Object.entries(persisted.phaseBudgets)) {
      if (!Number.isFinite(value.budget) || value.budget <= 0) continue;
      if (!Number.isFinite(value.spent) || value.spent < 0) continue;
      restored[title] = { budget: value.budget, spent: value.spent, warned: value.warned === true };
    }
    return restored;
  }
  ...
  for (const agent of persisted.agents ?? []) {
    ...
    const tokens = agent.tokenUsage?.total ?? agent.tokens ?? 0;
    if (!Number.isFinite(tokens) || tokens <= 0) continue;
    let phaseBudget = restored[agent.phase];
    if (!phaseBudget) {
      phaseBudget = { budget: Number.MAX_SAFE_INTEGER, spent: 0, warned: false };
      restored[agent.phase] = phaseBudget;
    }
...
export class WorkflowManager extends EventEmitter {
  ...
  constructor(options: WorkflowManagerOptions = {}) {
    super();
    this.cwd = canonicalizeWorkflowCwd(options.cwd ?? process.cwd());
  ...
  /** Emit observer-only lifecycle diagnostics without letting listeners change control flow. */
  private emitDiagnostic(eventName: string, event: unknown): void {
    try {
      this.emit(eventName, event);
    } catch {
      // Manager listeners are best-effort diagnostics. Durable state and run
      // ownership must not depend on consumer callback behavior.
    }
  }
    ...
    try {
      for (const p of this.listAllRuns()) {
        try {
          ...
          console.warn("[workflow] skipping malformed persisted run during stale-run recovery");
        }
      }
    } catch {
  ...
  private resolveExecutionPolicy(exec: ExecOptions = {}, legacy?: PersistedRunState): PersistedExecutionPolicy {
    ...
    if (persisted) {
      return {
        ...persisted,
        cwd: canonicalizeWorkflowCwd(persisted.cwd, { persisted: true }),
        maxAgents: normalizeMaxAgents(persisted.maxAgents),
        concurrency: normalizeConcurrency(persisted.concurrency),
        agentRetries: normalizeRetries(persisted.agentRetries),
      };
    }
    const requestedCwd = exec.cwd ?? legacy?.cwd ?? this.cwd;
    const cwd = canonicalizeWorkflowCwd(requestedCwd, { persisted: legacy?.cwd === requestedCwd });
    return {
      cwd,
      maxAgents: exec.maxAgents === undefined ? MAX_AGENTS_PER_RUN : normalizeMaxAgents(exec.maxAgents),
  ...
  startInBackground(
    ...
    const managed: ManagedRun = {
      ...
      generation: 1,
      journal: [],
      phaseBudgets: createPhaseBudgetRecord(),
      background: true,
      lease,
    ...
    const promise = this.trackExecution(managed, this.executeRun(managed, script, normalizedArgs, exec));
    promise.catch(() => {});
  ...
  async runSync(script: string, args?: unknown, exec: ExecOptions = {}): Promise<WorkflowRunResult> {
    const normalizedArgs = normalizeWorkflowArgs(args);
    const executionPolicy = this.resolveExecutionPolicy(exec);
    const managed = this.createManaged(script, normalizedArgs, executionPolicy);
    ...
    return this.trackExecution(managed, this.executeRun(managed, script, normalizedArgs, exec));
  }
  ...
  private async executeRun(
    ...
    const progress = () => {
      try {
        ...
        // Progress rendering is observer-only and must not alter run semantics.
      }
    };
    const updateTokenUsage = (usage: WorkflowTokenUsage, persistAttempt = false) => {
      if (!owns()) return;
      managed.snapshot.tokenUsage = usage;
      // A finalized attempt may belong to an atomic child whose parent journal
      // does not exist yet. Persist its cumulative accounting immediately so a
      // process crash restores the real spent budget instead of granting it back.
      // save() is synchronous and atomic, which serializes parallel completions.
      if (persistAttempt) this.persistRun(managed, true);
      this.emitDiagnostic("tokenUsage", { runId: managed.runId, usage });
      progress();
    };
    ...
    try {
      const result = await runWorkflow(script, {
        ...
        initialPhaseBudgets: resumeJournal ? managed.phaseBudgets : undefined,
        confirm,
        ...
        onLog: (message) => {
          ...
          this.emitDiagnostic("log", { runId: managed.runId, message });
          progress();
        },
        onPhaseBudgetProgress: (title: string, phaseBudget: WorkflowPhaseBudget) => {
          if (!owns()) return;
          managed.phaseBudgets[title] = { ...phaseBudget };
        },
        onPhaseState: (title) => {
          if (!owns()) return;
          managed.snapshot.currentPhase = title;
          if (title !== undefined && !managed.snapshot.phases.includes(title)) {
            managed.snapshot.phases.push(title);
          }
          if (title !== undefined) this.emitDiagnostic("phase", { runId: managed.runId, title });
          progress();
        },
        onAgentStart: (event) => {
          if (!owns()) return;
          ...
          this.emitDiagnostic("agentStart", { runId: managed.runId, ...event });
          progress();
        },
        onAgentEnd: (event) => {
          if (!owns()) return;
          ...
          this.emitDiagnostic("agentEnd", { runId: managed.runId, ...event });
          progress();
        },
        onAgentHistory: (event) => {
          ...
          this.emitDiagnostic("agentHistory", { runId: managed.runId, ...event });
          progress();
        },
        onTokenUsageProgress: (usage) => updateTokenUsage(usage, true),
      ...
      // A workflow script can catch the runtime's abort error and return normally.
      // The controller/status are the authoritative control-plane lifecycle: pause,
      // stop, or an external abort must win over that script return.
      ...
      this.emitDiagnostic("complete", { runId: managed.runId, result });
      ...
      if (usageLimitPaused) {
        ...
        this.emitDiagnostic("error", { runId: managed.runId, error: workflowError });
      }
  ...
  pause(runId: string): boolean {
    ...
    managed.status = "paused";
    managed.leaseReleaseBlocked = Boolean(managed.lease);
    this.emitDiagnostic("paused", { runId });
    this.persistRun(managed);
  ...
  async resume(runId: string, options: ResumeOptions = {}): Promise<boolean> {
    const hasArgs = Object.hasOwn(options, "args") && options.args !== undefined;
    ...
    try {
      ...
      let resumedArgs = normalizeWorkflowArgs(hasArgs ? options.args : persisted.args);
      if (hasArgsPatch) {
      ...
      const managed: ManagedRun = {
        ...
        phaseBudgets: restorePhaseBudgets(persisted),
        background: true,
        lease,
      ...
      this.emitDiagnostic("resumed", { runId });
  ...
  /** Acquire ownership before deleting any in-memory run that no longer holds its execution lease. */
  private acquireRunLeaseForDeletion(managed: ManagedRun): boolean {
    if (managed.lease) return true;
    ...
    try {
      ...
      if (!lease) return false;
      const persisted = this.persistence.load(managed.runId);
      if (!persisted || (managed.status === "paused" && persisted.status !== "paused")) {
        this.persistence.releaseRunLease(lease);
        return false;
      }
      managed.lease = lease;
  ...
  /** Delete a cold persisted run only while holding this process's token-owned lease. */
  private deletePersistedRunWithLease(runId: string): boolean {
    let lease: RunLease | null = null;
    try {
      lease = this.persistence.acquireRunLease(runId);
      if (!lease || !this.persistence.load(runId)) return false;
      return this.persistence.delete(runId, { preserveLock: true });
    } catch {
      return false;
    } finally {
      if (lease) this.persistence.releaseRunLease(lease);
  ...
  stop(runId: string): boolean {
    ...
    managed.status = "aborted";
    managed.leaseReleaseBlocked = Boolean(managed.lease);
    this.emitDiagnostic("stopped", { runId });
    this.persistRun(managed);
    void this.scheduleCancellationCleanup(managed);
    return true;
  }
  ...
  deleteRun(runId: string): boolean {
    const managed = this.runs.get(runId);
    if (!managed) return this.deletePersistedRunWithLease(runId);
    if (!this.acquireRunLeaseForDeletion(managed)) return false;
